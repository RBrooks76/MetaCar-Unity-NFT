    ...
    public override void Connect(SIOAuthPayload authPayload)
    {
        ...
        Task.Run(async () =>
        {
            ...
            try
            {
                ...
                String randomStr = rand.Next().ToString();
                ...
                Uri connectTarget = new Uri(baseUri.Scheme + "://" + baseUri.Host + ":" + baseUri.Port + baseUri.AbsolutePath + "?EIO=4&transport=websocket" + (baseUri.Query.Length > 1 ? "&" + baseUri.Query.Substring(1) : "") + "&t=" + randomStr);
